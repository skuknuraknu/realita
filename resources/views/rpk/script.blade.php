<script type="text/javascript">
        $(document).ready(function() {
              ...
              $('#tabel').DataTable({
                ...
                $('#tes').click(function() {
                  $("#rpk").append('<tr> <td></td> <td></td> <td><select name="kode_ik" type="text" id="kode_ik" class="kode_ik d-inline form-control w-auto required"> <option value="SILAHKAN PILIH"  selected="true">SILAHKAN PILIH</option> @foreach($KK as $dataKK) <option value="{{$dataKK->kode_ik}}">{{$dataKK->kode_ik}}</option>  @endforeach </select></td> <td class="indikator_kinerja" contenteditable="false"></td> <td><select name="kode_prog" class="kode_prog form-control d-inline w-auto required" id="">  <option value="SILAHKAN PILIH" >SILAHKAN PILIH</option></select></td> <td class="program" contenteditable="false"></td> <td> <select name="rincian_program" class="rincian_program form-control d-inline w-auto required" id="">  <option value="SILAHKAN PILIH" >SILAHKAN PILIH</option>  @foreach($RINCIANPROGRAM as $dataMAK) <option value="{{$dataMAK->Rip}}" >{{$dataMAK->Rip}}</option> @endforeach</select></td> <td contenteditable="true"></td> <td> <div id="uploadStatus"></div> <form id="uploadForm" enctype="multipart/form-data"> <input type="file" id="attachment" name="file" id="fileInput"> <input type="submit" class="submit" value="Submit" /></form></td> <td contenteditable="true"></td> <td><select name="Rencana_Jadwal_Pelaksanaan" class="Rencana_Jadwal_Pelaksanaan d-inline form-control w-auto required"><option value="TRIWULAN 1">TRIWULAN 1</option><option value="TRIWULAN 2">TRIWULAN 2</option><option value="TRIWULAN 3">TRIWULAN 3</option><option value="TRIWULAN 4">TRIWULAN 4</option> </select></td> <td><select name="tahun" class="tahun d-inline form-control w-auto required"><option value="PILIH TAHUN">PILIH TAHUN</option><option value="2022">2022</option><option value="2023">2023</option><option value="2025">2024</option><option value="2025">2025</option><option value="2026">2026</option></select></td> <td><span class="del_btn"><i role="button" class="rounded bg-danger text-white mr-1 p-3 fa-solid fa-trash fa-sm"></i></span><span class="save_btn"><i role="button" class="text-white mr-1 rounded bg-info p-3 fa-solid fa-floppy-disk fa-sm"></i></span><span class="new_btn"><i role="button" class="text-white mr-1 rounded bg-success p-3 fa-solid fa-plus fa-sm"></i></span></td></tr>');
                });
              // //ambil setiap nilai td
             $('table > tbody  > tr').each(function(index, tr) {
                   
                    let self = $(this);
                    let kode_ik = self.find("#ik");
                       ...
                       if($(this).is(':nth-child(3)')){
                        ...
                        if($(this).is(':nth-child(5)')){
                         ...
                         if($(this).is(':nth-child(7)')){
                            $(this).html('<select  style="font-size: 10px; font-weight: bold;" name="rincian_program" class="rincian_program form-control d-inline w-auto required" id="">  <option value="SILAHKAN PILIH" >SILAHKAN PILIH</option>  @foreach($RINCIANPROGRAM as $dataMAK) <option value="{{$dataMAK->Rip}}" >{{$dataMAK->Rip}}</option> @endforeach</select>')
                        }
